...
def makeSVG(data):
    ...
    if data == {}:
        ...
        if recent_plays == {}:
            # Handle no recent plays, e.g., return empty or default
            item = {
                "album": {"images": [{"url": ""}], "name": ""},
                "artists": [{"name": "No tracks available"}],
                "name": "",
            }
        else:
        ...
        item = data["item"]
    img = (
        loadImageB64(item["album"]["images"][1]["url"])
        if item["album"]["images"]
        else ""
    )
    artistName = (
        item["artists"][0]["name"].replace("&", "&amp;") if item["artists"] else ""
    )
    songName = item["name"].replace("&", "&amp;") if "name" in item else ""
    dataDict = {
